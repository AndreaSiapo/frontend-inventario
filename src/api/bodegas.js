...
export async function getBodegas({ search = '', perPage = 10, page = 1, orderBy = 'id', orderDir = 'asc' } = {}) {
...
export function getBodega(id) {
  return apiGet(`/bodegas/${id}`);
}

export function createBodega(data) {
  return apiPost("/bodegas", data);
}

// si tuvieras editar
export function updateBodega(id, data) {
  return apiPut(`/bodegas/${id}`, data);
}

// si tuvieras eliminar
export function deleteBodega(id) {
  return apiDelete(`/bodegas/${id}`);
}
